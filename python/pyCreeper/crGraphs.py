...
def createBarPlot(data_,
      title_="", xLabel_ = "", yLabel_ = "", xTickLabels_=[], legendTitle_ = "", legendLabels_ = [],
      showConfidenceIntervals_=False, showAverages_=False, averagesIgnoreZeros_=False, barWidth_ = 0.8,
      yMin_=INVALID_VALUE, yMax_=INVALID_VALUE, yTicksStep_ = 0, yTicksStepMultiplier_ = 1,
   ...
   :param `data_`: A 2D or a 3D list of numbers. The 0th dimension represents individual bar groups (colors). The 1st dimension represents data points along the x axis. Optional 3rd dimension is a list of values that each bar consists of. A bar then shows a median / average of that list
   ...
   :param `xTickLabels_`: (optional, default = []) A 1D list of tick labels for the x-axis.
   :param `legendTitle_`: (optional, defalut = "") Text shown before legend
   ...
   :param `showConfidenceIntervals_`: (optional, default = False) A boolean that specified whether to show error bars. If True, `data_` must be a 3D list
   :param `showAverages_`: (optional, default = False) A boolean that specifies whether to show averages instead of medians. If True, `data_` must be a 3D list
   ...
   if (len(xTickLabels_) > 0):
      crHelpers.checkListHasAtLeastLengthOfList(xTickLabels_, data_[0]);
   ...
   #-- adjust bar width based on the number of bars that will be shown for one tick
   barWidth = barWidth_ / len(data_);

   fig, ax = createFigure(size_, title_, figure_, subPlot_, xLabel_, yLabel_, yStretchMultiply_=1.1);
   ...
   for i in range(len(data_)):
      ...
      barLocations = xLocations + (i - math.floor(len(data_)/2))*barWidth;

      if (numOfDataDimensions == 3):
         shownValues = [];
         for q in range(len(data_[i])):
            if (showAverages_):
               shownValues.append(crData.getAverageOfAList(data_[i][q], averagesIgnoreZeros_));
            else:
               shownValues.append(numpy.median(data_[i][q]));
         ...
         if (showConfidenceIntervals_):
            dataDof = [(len(data_[i][q])-1) for q in range(len(shownValues))]; #degrees of freedom is sample size -1
            dataStd = [numpy.std(data_[i][q]) for q in range(len(shownValues))];

            plot = ax.bar(barLocations, shownValues, barWidth, color=colorCode, yerr=scipy.stats.t.ppf(0.95, dataDof)*dataStd); #yerr=stdData_[i]
            plots.append(plot);

         else:
            plot = ax.bar(barLocations, shownValues, barWidth, color=colorCode);
            plots.append(plot);
         ...
         plot = ax.bar(barLocations, data_[i], barWidth, color=colorCode); #yerr=stdData_[i]
         plots.append(plot);
   ...
   xMin_ = INVALID_VALUE;
   xMax_ = INVALID_VALUE;
   if (xMin_ == INVALID_VALUE):
      xMin_ = min(-0.5, 0 - barWidth_*0.75);
   if (xMax_ == INVALID_VALUE):
      xMax_ = max(len(data_[0]) - 0.25, len(data_[0]) - barWidth_*0.5);
...
def createLinePlot(data_,
   ...
   :param `showConfidenceIntervals_`: (optional, default = False) A boolean that specified whether to show error bars around data points. If True, `data_` must be a 3D list
...
def setTickLabels(ax_, data_, xTickLabels_, minSpacingBetweenXLabels_):

   size_ = getStyle().figureSize;
   tickFontSize_ = getStyle().tickFontSize;
   labelFontSize_ = getStyle().labelFontSize;

   xLocations = numpy.arange(len(data_[0]))  # the x locations for the ticks
   xTickLabels = xTickLabels_.copy();

   #-- if user didn't define tick labels and too many tick labels would be produced automatically, adjust their frequency
   if (len(xTickLabels_) == 0):
      xTickLabels = [];
      xTickFrequency = max(1,math.floor(len(xLocations)/(size_[0]*0.9)));
      #print(xTickFrequency)
      spacing = (size_[0]*0.9) / len(xLocations);
      #print(spacing, minSpacingBetweenXLabels_);
      if (spacing < minSpacingBetweenXLabels_):
         #-- make sure the tick frequency is a nice round number
         allowedFrequences = [1,2,5,10,25,50,100,250,500,1000,2500,5000,10000,25000,50000,100000];
         for f in allowedFrequences:
            if (xTickFrequency < f):
               xTickFrequency = f;
               break;
      else:
      ...
      #-- create numbered ticks of proper frequency, also marking the last tick value
      for i in range(len(xLocations)):
         val = i+1 # making sure xticks start at 1 not 0
         #--  each i ticks               that are not too close to the end         plus the first     and last tick
         if ((val%xTickFrequency == 0 and val < len(xLocations) - xTickFrequency/2 ) or (val == 1) or (val == len(xLocations))):
            xTickLabels.append(val);
         else:
            xTickLabels.append('');

   #-- apply the labels
   plt.xticks(xLocations, size=labelFontSize_);
   ax_.set_xticklabels(xTickLabels, size=tickFontSize_);
